...
export function EditUserForm({ user, onUserUpdate, open, onOpenChange }: EditUserFormProps) {
  ...
  }, [user, form, open]); // Added `open` to deps to ensure reset on reopen
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const updatedValues: Partial<User> = { ...values };
    // Don't send an empty password. If it's empty, it means no change.
    if (!updatedValues.password) {
        delete updatedValues.password;
    }
    onUserUpdate({ ...user, ...updatedValues });
  }
